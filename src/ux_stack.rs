    ...
    async fn handle_chat_async(req_str: String, tx: tokio::sync::broadcast::Sender<String>, cancel_token: std::sync::Arc<std::sync::atomic::AtomicBool>) {
        ...
        let mut messages = vec![
            crate::Message { 
                ...
                content: format!("You are an autonomous AI coding agent running on the '{}' model. You can execute bash commands and read/write files to solve user requests. Once you have solved the request, output your final response in Markdown without using any tools. When conversing or answering questions, output plain text or Markdown. Do NOT output empty JSON objects like `{{}}`.", selected_model), 
                tool_calls: None, 
                images: None 
            }
        ];
